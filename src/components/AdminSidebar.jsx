import { NotebookText,Folder,User,Bell,KeyRound,SquareArrowOutUpRight,LogOut } from 'lucide-react';
 function SideBar({pageNow}) {
    const styleSelectedMenu="px-6 py-3 h-16 bg-[#E4E0DA] font-medium text-[#43403B] text-lg flex items-center gap-3 "
    const styleSelectableMenu ="px-6 py-3 h-16 hover:bg-gray-200 font-medium text-[#75716B] text-lg flex items-center gap-3 cursor-pointer"
    console.log(pageNow)
    return (
      <div className="h-screen w-[280px] bg-[#F6F5F3] flex flex-col justify-between py-4 ">
        {/* Top section */}
        <div>
        ...
        <div className="mb-4">
          ...
          <SquareArrowOutUpRight size={22}/> hh. website
          </div>
          <div className={styleSelectableMenu}>
            <LogOut size={22} className="rotate-180"/> Log out
          </div>
        </div>
      </div>
    );
